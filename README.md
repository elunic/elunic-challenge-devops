...
1. You MAY NOT abuse the server in any way or use it for purposes unrelated to the challenge (obviously)
2. You MAY NOT modify the Terraform code or the Kubernetes manifests that deploys the initial cluster, since it deploys in a way that sets up some of the tasks - BUT if the initial deployement fails, feel free to debug and fix it.
3. Clone the repo to YOUR local machine, not the remote server.
...
A shell script `./shell` is provided in the root directory that sets up a predefined Docker-based shell environment with all necessary CLI tools pre-installed in the correct versions. On the first run it will build.

The shell script is not mandatory; it simply provides an environment with the necessary tools in the correct versions.
