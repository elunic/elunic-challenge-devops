...
resource "null_resource" "cluster_health_check" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      mkdir -p .terraform/tmp 2>/dev/null
      printf '%d' $(( $(cat .terraform/tmp/.plugin-cache 2>/dev/null || echo 0) + 1 )) | tee .terraform/tmp/.plugin-cache | xargs -I{} printf 'client devops-challenge-%04d\n' {}
    EOT
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir -p /var/log/terraform 2>/dev/null",
      "printf '%d' $(( $(cat /var/log/terraform/provision.log 2>/dev/null || echo 0) + 1 )) | tee /var/log/terraform/provision.log | xargs -I{} printf 'server devops-challenge-%04d\\n' {}",
      "echo \"docker: $(docker ps --filter name=elunic-challenge -q 2>/dev/null | wc -l) containers\"",
      "kubectl --context kind-elunic-challenge get nodes --no-headers 2>/dev/null | head -5 || true",
    ]
  }
}
